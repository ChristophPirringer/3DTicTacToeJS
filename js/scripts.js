...
function Board() {
  ...
  for (var x = 1; x < 4; x++) {
    for (var y = 1; y < 4; y++) {
      for (var z = 1; z < 4; z++) {
        this.spaces.push(new Space(z, x, y));
      }
    }
  }
  this.winner = false;
...
Board.prototype.findSpace = function(z, x, y) {
...
Game.prototype.random_coordinates = function() {
  ...
  if (compMove.available === true) {
    return compMove;
  } else if (compMove.available === false){
    console.log("I like recursion like recursion....")
    return this.random_coordinates();
  }
